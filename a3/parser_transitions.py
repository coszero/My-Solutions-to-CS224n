...
def minibatch_parse(sentences, model, batch_size):
    ...
    partialParses = [PartialParse(sen) for sen in sentences]
    unfinishedParses = partialParses[:]

    while unfinishedParses:
        miniBatch = unfinishedParses[:batch_size]
        transitions = model.predict(miniBatch)
        for m,t in zip(miniBatch,transitions):
            m.parse_step(t)
        unfinishedParses = [m for m in unfinishedParses if not (len(m.stack) == 1 and len(m.buffer) == 0)]

    dependencies = [m.dependencies for m in partialParses]
