...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(input_size=word_embed_size,
                               ...
                               bias=True,
                               bidirectional=True)
        self.decoder = nn.LSTMCell(input_size=(word_embed_size + hidden_size),
                                   hidden_size=hidden_size,
                                   bias=True)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)  # (src_len, b, w_len)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)  # (tgt_len, b, w_len)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings_source(source_padded)  # source_padded (s_len,b), X (s_len,b,e)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(target_padded)  # (t_len,b,e)
