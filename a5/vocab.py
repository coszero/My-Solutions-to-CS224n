...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent_char = self.words2charindices(sents)   # (List[List[List[int]]])
        padded_sent_char = torch.tensor(pad_sents_char(sent_char, self.char_pad)
                                        , dtype=torch.long, device=device)   # (b, s_len, w_len)
        sents_var = padded_sent_char.permute(1, 0, 2)   # (s_len, b, w_len)
